...
//static const int OUTPUT_SIZE = Yolo_v6::MAX_OUTPUT_BBOX_COUNT * sizeof(Yolo_v6::Detection) / sizeof(float) + 1;  // we assume the yololayer outputs no more than MAX_OUTPUT_BBOX_COUNT boxes that conf >= 0.1
//
//struct yolodetect
//{
//	char* trtModelStream;
...
//	uint8_t* img_device;
//};
class yolov5_v6_inf:public basic_yolo
{
public:
	//trt_basic_config basic_config;
	yolov5_v6_inf();
	~yolov5_v6_inf();
	int init(trt_basic_config input_config, std::string modelpath);//≥ı ºªØ
