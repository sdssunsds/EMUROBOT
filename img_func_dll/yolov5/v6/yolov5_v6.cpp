...
cv::Rect yolov5_v6_inf::get_rect_v6(int w,int h, float bbox[4]) {
    float l, r, t, b;
    float r_w = basic_config.INPUT_W / (w * 1.0);
    float r_h = basic_config.INPUT_H / (h * 1.0);
    if (r_h > r_w) {
        l = bbox[0] - bbox[2] / 2.f;
        r = bbox[0] + bbox[2] / 2.f;
        t = bbox[1] - bbox[3] / 2.f - (basic_config.INPUT_H - r_w * h) / 2;
        b = bbox[1] + bbox[3] / 2.f - (basic_config.INPUT_H - r_w * h) / 2;
        l = l / r_w;
        r = r / r_w;
        t = t / r_w;
        b = b / r_w;
        ...
        l = bbox[0] - bbox[2] / 2.f - (basic_config.INPUT_W - r_h * w) / 2;
        r = bbox[0] + bbox[2] / 2.f - (basic_config.INPUT_W - r_h * w) / 2;
        t = bbox[1] - bbox[3] / 2.f;
        b = bbox[1] + bbox[3] / 2.f;
        l = l / r_h;
        r = r / r_h;
...
int yolov5_v6_inf::init(trt_basic_config input_config, std::string modelpath)
{

    cudaSetDevice(DEVICE);
    basic_config = input_config;
    std::ifstream file(modelpath+
        input_config.engine_name+".engine", std::ios::binary);
    if (! file.good()) {
    ...
    CUDA_CHECK(cudaMalloc((void**)&v6_dec.buffers[v6_dec.inputIndex], BATCH_SIZE * 3 * basic_config.INPUT_H * basic_config.INPUT_W * sizeof(float)));
...
std::vector<inf_res> yolov5_v6_inf::do_infer(cv::Mat input)
{
    ...
    preprocess_kernel_img_v6(v6_dec.img_device, inputcopy.cols, inputcopy.rows, buffer_idx, basic_config.INPUT_W, basic_config.INPUT_H, v6_dec.stream);
    // Run inference
    ...
    nms(batch_res, &v6_dec.prob[0], basic_config.CONF_THRESH, basic_config.NMS_THRESH);
    ...
    if (batch_res.size()>0)
    {
        for (size_t j = 0; j < batch_res.size(); j++) {
            ...
            if (w>0&&h>0)
            {
                ...
                res_info_s.box_name = basic_config.classname[(int)batch_res[j].class_id];
