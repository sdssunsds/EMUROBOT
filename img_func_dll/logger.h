...
namespace LOGGER
{
	...
	public:
		//写严重错误信息
		void TraceFatal(std::string lpcszFormat, ...);
		//写错误信息
		void TraceError(std::string lpcszFormat, ...);
		//写警告信息
		void TraceWarning(std::string lpcszFormat, ...);
		//写提示信息
		void TraceInfo(std::string lpcszFormat, ...);
		//改变写日志级别
