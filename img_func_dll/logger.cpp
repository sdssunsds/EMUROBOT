...
namespace LOGGER
{
	...
	//写严重错误信息
	void CLogger::TraceFatal(std::string lpcszFormat, ...)
	{
		//判断当前的写日志级别
		if (EnumLogLevel::LogLevel_Fatal > m_nLogLevel)
			return;
		string strResult;
		if (NULL != lpcszFormat.c_str())
		{
			va_list marker = NULL;
			va_start(marker, lpcszFormat); //初始化变量参数
			size_t nLength = _vscprintf(lpcszFormat.c_str(), marker) + 1; //获取格式化字符串长度
			std::vector< char> vBuffer(nLength, '\0'); //创建用于存储格式化字符串的字符数组
			int nWritten = _vsnprintf_s(&vBuffer[0], vBuffer.size(), nLength, lpcszFormat.c_str(), marker);
			if (nWritten > 0)
			{
				strResult = &vBuffer[0];
			}
	...
	//写错误信息
	void CLogger::TraceError(std::string lpcszFormat, ...)
	{
		//判断当前的写日志级别
	...
	//写警告信息
	void CLogger::TraceWarning(std::string lpcszFormat, ...)
	{
		//判断当前的写日志级别
	...
	//写一般信息
	void CLogger::TraceInfo(std::string lpcszFormat, ...)
	{
		//判断当前的写日志级别
